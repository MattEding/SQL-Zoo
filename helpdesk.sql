...
SELECT COUNT(*) AS mngr_lvl_call_cnt
FROM Issue AS i
...
SELECT SUM(count_per_caller) / AVG(total_calls) * 100 AS top_20_pct
FROM (SELECT
        count_per_caller,
        total_calls,
        total_rows,
        @n := @n + 1 AS row
      FROM (SELECT COUNT(*) AS count_per_caller
            FROM Issue
            GROUP BY caller_id
            ORDER BY COUNT(*) DESC) AS call_counts_desc
      JOIN (SELECT @n := 0) AS row_init
      JOIN (SELECT
              SUM(cnt) AS total_calls,
              COUNT(cnt) AS total_rows
            FROM (SELECT COUNT(*) AS cnt
                  FROM Issue
                  GROUP BY caller_id) AS call_counts) AS totals) AS _
WHERE row / total_rows <= 0.2
